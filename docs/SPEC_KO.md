# MIRROR-SPEC v1.0 — 미러스택 원장 포맷 & 검증 프로토콜
...
3. 다이제스트는 소문자 hex 16자로 절단해야 한다(MUST).
4. 엔트리는 추가로 Ed25519 서명을 실을 수 있다(MAY). `sig`는 **16자 소문자
   hex 봉인 문자열의 UTF-8 바이트**(raw 다이제스트가 아님)에 대한
   소문자-hex Ed25519 서명이고; `pubkey`는 소문자-hex 32바이트 raw 공개키다.
   `pubkey`는 봉인 대상 body의 일부이고(IS); `sig`는 아니다.
5. 봉인은 유일성을 보장하지 않는다: 64비트 절단은 원리적으로 충돌을
   허용하고, 동일한 엔트리는 동일한 봉인을 만든다. 검증기는 봉인 중복을
   검사하지 않는다; 엔트리의 정체성은 봉인 값이 아니라 체인 위치(§5)다.

*절단에 관한 주:* hex 16자(64비트)는 변조-증거(tamper-evidence) 체크섬이지,
적대적 preimage 탐색에 견디는 충돌 저항 커밋먼트가 아니다. 파일 전체
SHA-256(§6.4, §6.5)이 전체 강도의 커밋먼트를 제공한다.
