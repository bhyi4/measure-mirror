...
| FAIL | pass 바가 우연 수준 이하(넘어도 아무것도 증명 못 함) |
...
| ㉗ | `prereg_lint` | 기형 봉인: kill-condition 누수, 우연 이하 바, 비구조화 kill, 저 n, 체크 미선언 | 독립 실행 (연산 전; MCP `mm_register`에서 자동) |
