...
- **실사례**: (한글 문자 언어모델 실험) DCC 한글 char-LM 0.32 plateau를 '천장'으로 단정 → 천장 아님(PHANTOM), 이중진범=비트BCE손실+ODE본체, 처방=자모CE헤드+보존ODE. 출처: db/curated/false_negative_guards.jsonl — dcc_plateau_metric_phantom
